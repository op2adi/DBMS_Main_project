...
def transport_data():
    for i in range(10):
        ...
        cursor.execute(insert_query, transport_data)
        adi_con.commit()
        print(f"INSERT INTO Transport (Transport_id, Start_Loc, Destn_Loc, Timings, Price, Vacany) VALUES {transport_data[0], transport_data[2], transport_data[2], transport_data[3], transport_data[4],transport_data[5]}"+";")


def fetch_transport_data():
    cursor.execute("SELECT Transport_id FROM Transport")  # Replace 'attribute_column' with the actual attribute column name
...
def generate_train_data(transport_ids):
    ...
    for i in range(5):
        ...
        transport_ids.remove(transport_id)  # Remove the used transport ID to ensure it's not used for flights
    return trains
